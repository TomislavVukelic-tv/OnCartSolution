...
public sealed class OutboxPublisher : BackgroundService
{
        ...
        Dictionary<string, List<string>> subscribers, int batchSize, CancellationToken token)
    {
        ...
        foreach (var evt in pending)
        {
            ...
            // Resolve the subscribers registered for this event type.
            if (!subscribers.TryGetValue(evt.EventType, out var targets) || targets.Count == 0)
            {
                // No one is listening for this event type: nothing to deliver,
                // mark it done so it doesn't linger in the queue forever.
            ...
            // A row is marked done only when EVERY subscriber accepts it.
            // If any subscriber fails, the row stays pending and the whole
            // event is retried next cycle. Because a retry re-POSTs to every
            // subscriber (we don't track per-subscriber success), each
            // consumer MUST dedupe on eventId.
